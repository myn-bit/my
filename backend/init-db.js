...
async function addTestData() {
    try {
        ...
        const admin2Exists = await database.get('SELECT id FROM users WHERE email = ?', ['admin2@pcstore.ru']);
        
        if (!admin2Exists) {
            const salt = bcrypt.genSaltSync(10);
            const admin2PasswordHash = bcrypt.hashSync('admin456', salt);
            
            await database.run(
                `INSERT INTO users (username, email, password_hash, role) 
                VALUES (?, ?, ?, ?)`,
                ['admin2', 'admin2@pcstore.ru', admin2PasswordHash, 'admin']
            );
            console.log('✅ Второй администратор: admin2@pcstore.ru (пароль: admin456)');
        }
        
        // Проверяем, есть ли уже тестовый пользователь
        const userExists = await database.get('SELECT id FROM users WHERE email = ?', ['user@pcstore.ru']);
        ...
        if (!userExists) {
            ...
            await database.run(
                `INSERT INTO users (username, email, password_hash, role) 
                VALUES (?, ?, ?, ?)`,
                ['user', 'user@pcstore.ru', userPasswordHash, 'user']
            );
